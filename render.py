...
def rest(path):
    """GET a REST endpoint. Returns (status, parsed body or None)."""
    req = urllib.request.Request(
...
def lines_of_code(repo_names):
    """
    Sum the user's additions/deletions across repos using the contributor
    stats endpoint (default branch only). GitHub computes these lazily and
    answers 202 until ready, so pending repos are retried in rounds.
    """
    added = deleted = 0
    pending = list(repo_names)
    for attempt in range(8):
        still_pending = []
        for name in pending:
            status, body = rest(f"/repos/{USER}/{name}/stats/contributors")
            if status == 202:
                still_pending.append(name)
            elif status == 200 and body:
                for contributor in body:
                    if (contributor.get("author") or {}).get("login", "").lower() == USER.lower():
                        added += sum(w["a"] for w in contributor["weeks"])
                        deleted += sum(w["d"] for w in contributor["weeks"])
            elif status not in (200, 204):
                print(f"warning: LOC stats for {name} returned {status}, skipped", file=sys.stderr)
        pending = still_pending
        if not pending:
            break
        time.sleep(5 + attempt * 5)
    if pending:
        print(f"warning: LOC stats still computing for {pending}, skipped this run", file=sys.stderr)
...
def fetch_stats():
    if not TOKEN:
        if os.environ.get("GITHUB_ACTIONS") == "true":
            sys.exit("GH_TOKEN is empty in the workflow; refusing to render placeholder stats")
        print("GH_TOKEN not set: rendering placeholder stats", file=sys.stderr)
        return {"repos": 0, "stars": 0, "commits": 0, "followers": 0,
                "added": 0, "deleted": 0, "created": "2023-01-01T00:00:00Z"}
    ...
    repos, stars, cursor, names = 0, 0, None, []
    while True:
        page = gql(
    ...
    print(f"repos={repos} stars={stars} commits={commits} +{added} -{deleted}")
...
def kv_segs(key, value_segs, width, kclass="k_id"):
    value_len = sum(len(t) for t, _ in value_segs)
    dots = max(1, width - len(key) - value_len - 2)
    return [(key, kclass), (" " + "." * dots + " ", "dots")] + value_segs
...
def info_lines(stats):
    ...
    lines += [
        ...
        kv_segs("Lines", [
            (f"{stats['added'] - stats['deleted']:,}", "v_stat"),
            (" (", "dots"), (f"+{stats['added']:,}", "v_add"),
            (" ", "dots"), (f"-{stats['deleted']:,}", "v_del"), (")", "dots"),
        ], INFO_COLS, "k_stat"),
